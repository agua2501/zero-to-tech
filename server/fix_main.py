...
FRONTEND_DIR = BASE_DIR.parent            # 期末作业/
...
def seed_data():
    routes = [
        ("露天矿生态公园一日游", "coastal", 120, 1, "img/1122.jpg",
         "游览茂名露天矿生态公园，欣赏碧波荡漾的湖水与城市绿肺，包含导游讲解和骑行体验"),
    ...
    news_data = [
        ("茂名荔枝文化节盛大开幕，邀您共享\u201c荔枝自由\u201d甜蜜盛宴", "2025-06-20"),
        ("投资近百亿！中煤茂名电厂项目落地零碳园区，产业群将迎稳定热源", "2025-06-18"),
        ("中国第一滩夏季旅游旺季开启，滨海休闲游热度攀升", "2025-06-15"),
        ("茂名扎实推进\u201c百千万工程\u201d，乡村振兴焕发新活力", "2025-06-12"),
        ("茂名冼太夫人故里文化旅游区获评省级研学基地", "2025-06-10"),
        ("茂名新增两条高速公路，进一步完善粤西交通网络", "2025-06-08"),
        ("广湛高铁茂名段建设提速，未来广州到茂名仅需1.5小时", "2025-06-05"),
        ("茂名打造\u201c中国月饼名城\u201d，月饼产业年产值突破百亿", "2025-06-01"),
        ("茂名\u201c好心文化\u201d品牌亮相深圳文博会，吸引大量关注", "2025-05-28"),
...
async def register(request: Request):
    ...
    if not username or not email or not password:
        raise HTTPException(400, {"ok": False, "msg": "请填写完整信息"})
    if conn.execute("SELECT id FROM users WHERE username = ?", (username,)).fetchone():
        raise HTTPException(400, {"ok": False, "msg": "用户名已被注册"})
    if conn.execute("SELECT id FROM users WHERE email = ?", (email,)).fetchone():
        raise HTTPException(400, {"ok": False, "msg": "邮箱已被注册"})
...
async def login(request: Request):
    ...
    if not username or not password:
        raise HTTPException(400, {"ok": False, "msg": "请输入用户名和密码"})
    user = conn.execute("SELECT * FROM users WHERE username = ?", (username,)).fetchone()
    if not user or user["password"] != hash_password(password):
        raise HTTPException(401, {"ok": False, "msg": "用户名或密码错误"})
...
async def me(request: Request):
    ...
    if not user:
        raise HTTPException(401, {"ok": False, "msg": "未登录或登录已过期"})
    return {"ok": True, "data": {"username": user["username"], "email": user["email"]}}

@app.get("/api/routes")
async def list_routes(destination: Optional[str] = Query(None), days: Optional[str] = Query(None), priceMin: Optional[float] = Query(None), priceMax: Optional[float] = Query(None)):
    ...
    return {"ok": True, "data": rows_to_list(conn.execute(sql + " ORDER BY id", params).fetchall())}

@app.get("/api/routes/{route_id}")
async def get_route(route_id: int):
    row = conn.execute("SELECT * FROM routes WHERE id = ?", (route_id,)).fetchone()
    if not row:
        raise HTTPException(404, {"ok": False, "msg": "线路不存在"})
...
async def book_route(route_id: int, request: Request):
    ...
    if not name or not phone:
        raise HTTPException(400, {"ok": False, "msg": "请填写姓名和电话"})
    now = datetime.now().strftime("%Y/%m/%d %H:%M:%S")
    conn.execute("INSERT INTO bookings (route_id, user_id, customer_name, phone, people, remark, created_at) VALUES (?, ?, ?, ?, ?, ?, ?)",
...
@app.get("/api/news")
async def list_news():
    return {"ok": True, "data": rows_to_list(conn.execute("SELECT * FROM news ORDER BY date DESC, id DESC").fetchall())}

@app.get("/api/news/{news_id}")
async def get_news(news_id: int):
    row = conn.execute("SELECT * FROM news WHERE id = ?", (news_id,)).fetchone()
    if not row:
        raise HTTPException(404, {"ok": False, "msg": "新闻不存在"})
    return {"ok": True, "data": dict(row)}
